...
class HeadphonePage(Base):
    ...
    @staticmethod
    def filter_brand_xpath(filter_brand):
        return f'//span[@class="filter__item-text-option"][contains(text(), "{filter_brand}")]'
    ...
    def get_filter_brand(self, filter_brand):
        return self.wait.until(ec.element_to_be_clickable((By.XPATH, self.filter_brand_xpath(filter_brand))))
    ...
    def filter_and_add_headphones_to_cart(self, filter_brand):
        with allure.step('Filter and add headphones to cart'):
            ...
            self.click_filter_brand(filter_brand)
